...
export const getHoleByIdTool: Tool = {
  name: 'get_hole_by_id',
  description:
    'Get type information and available bindings in scope for a hole by its ID. If you do not know the ID, find it using the list_holes tool.',
  inputSchema: {
    type: 'object',
    properties: {
      uri: {
...
function truncateScope(scope: ScopeInfo): {
  bindings: { name: string; signature?: string }[];
  truncated: boolean;
} {
  ...
  const bindings = filteredBindings.slice(0, 10).map((b) => ({
    name: b.name,
